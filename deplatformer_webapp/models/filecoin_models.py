...
class FfsUser(db.Model):
    ...
    ffs_userid = db.Column(db.String(36), index=True,)
    ...
    def __init__(
        self, ffs_userid, token, creation_date, user_id,
    ):
...
class Files(db.Model):
    ...
    user_id = db.Column(db.Integer(), db.ForeignKey(User.id), nullable=False,)
    ffs_id = db.Column(db.Integer(), db.ForeignKey(FfsUser.id), nullable=False,)

    def __init__(
        self, file_path, file_name, upload_date, file_size, CID, platform, user_id, ffs_id,
    ):
...
class Wallets(db.Model):
    ...
    def __init__(
        self, created, address, ffs, user_id,
    ):
